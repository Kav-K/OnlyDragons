...
/** Independent real-Paper rejection, native retry, legacy and lifecycle conservation. */
public final class AnvilBoundaryScenario implements Scenario, Listener {
    ...
    private static ItemStack copy(ItemStack s){return empty(s)?null:s.clone();}private static boolean empty(ItemStack s){return s==null||s.getType().isAir();}private static int amount(ItemStack s){return empty(s)?0:s.getAmount();}
