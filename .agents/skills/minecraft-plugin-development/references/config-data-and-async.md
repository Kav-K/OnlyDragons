...
- retain dirty updates made while a snapshot is being saved; clear only the saved revision after successful persistence, and handle failures without discarding pending writes
...
- reject stale async results using the current session/request generation after reconnect or reload, and handle shutdown before scheduling gameplay callbacks
