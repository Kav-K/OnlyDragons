...
- use `plugin.yml` declarations for Bukkit-style command registration; preserve native Paper Brigadier/lifecycle registration when that is the project's model
- if a command may be optional or renamed, null-check `getCommand`
- fail startup with an actionable error if a required Bukkit-style command is absent rather than silently omitting registration
...
Stop accepting new writes before the final flush; account for already-running async work and use a bounded completion strategy. Do not enqueue a server-thread callback and block that same thread waiting for it during disable.
