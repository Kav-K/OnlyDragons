...
- verify the world unloaded successfully before deleting its folder
- resolve the final path and require a plugin-owned temporary instance under the configured instance directory; never delete the source template, server root, or an unrelated world
- keep ownership explicit across restarts so abandoned temporary instances can be identified safely; log unload or deletion failures and retain the folder when ownership is uncertain
...
- declare hard dependencies only when startup cannot work without them, using `depend` for Bukkit-style metadata or native Paper's dependency model
- use `softdepend` (or native Paper's optional dependencies) and runtime checks for optional features
