...
[practice combat loop](dev/combat-play.md), and [managed test-dragon controls](dev/dragon-play.md).
The controls include post-kill ranking, bounded orbit/stationary modes and a named
returning-Tracer calibration bow. The [checkpoint](docs/planning/07-checkpoint-two.md)
also plans dragon health UI, revised Tempo ghost damage, held-fire loadouts and
ten-enchant XP-cost books. Armor effects and real rewards remain out of scope. See the
