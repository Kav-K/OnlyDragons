...
| T06 — Firing/Duplex | In review; automated gates pending | [#9](https://github.com/Kav-K/OnlyDragons/issues/9), [draft PR #52](https://github.com/Kav-K/OnlyDragons/pull/52) | Production owned firing/session/settlement and two-player fixtures implemented; 181 unit tests pass. The reviewed lifecycle oracle separates actual client quit from synthetic pre-settlement session cleanup. [API, calibration and evidence](evidence/t06-firing.md). The corrected clean feature run and complete current-main T06 cohort/checkpoint remain required. |
