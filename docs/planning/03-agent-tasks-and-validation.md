...
GH-5 continuation starts from main `5b0e030`, containing merged T09b PR #27.
The additive `projectile-player-feasibility` scenario uses the unchanged fixed
client sequence and explicitly distinguishes its native bow release from later
API-spawned arrows owned by that connected player. Only that exact additional
scenario ID is admitted to protocol mode; production and client contracts remain
unchanged. Native positive/cancellation controls and geometry/phase observations
are awaiting actual Paper evidence. #9 remains blocked and M0 unaccepted.
