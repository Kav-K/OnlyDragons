...
| T06 — Firing/Duplex | In review; automated gates pending | [#9](https://github.com/Kav-K/OnlyDragons/issues/9), [draft PR #52](https://github.com/Kav-K/OnlyDragons/pull/52) | Production owned firing/session/settlement and two-player fixtures implemented; 178 unit tests pass. Clean focused `3f10f61` verifies part/phase/input controls but fails the pending-quit window. [API, calibration and evidence](evidence/t06-firing.md). Expected-server-kick fixture coordination, passing feature run and the complete current-main T06 cohort/checkpoint remain required. |
