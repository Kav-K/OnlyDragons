...
Baseline reconciled **5 September 2026** against main `1efa7d0` and repository
issue/PR state. The plugin still supplies starter status/reload commands and
welcome messages. T00 contracts, the T01a stat resolver, and the T09a isolated
Linux/WSL Paper runner are integrated. Combat implementation is active in #6;
...
| T03 — Combat/ledger | In review | [#6](https://github.com/Kav-K/OnlyDragons/issues/6), [draft PR #26](https://github.com/Kav-K/OnlyDragons/pull/26), `symphony/gh-6` | Clean 0dd0fa7 includes main 7c6d843: 74 production tests, 27 runner tests and 31 combat-accounting Paper assertions passed. [Evidence](#t03-combat-validation). Lead review/CI/merge pending; physical adapter/native suppression and human checks remain separate. |
| T04 — Paper feasibility | Blocked (partial implementation integrated) | [#5](https://github.com/Kav-K/OnlyDragons/issues/5), [PR #22](https://github.com/Kav-K/OnlyDragons/pull/22) | Partial PR #22 merged at 586a170 after review, final-head CI, 34 Paper assertions and expected exception/abort controls; [evidence](../../dev/game-tests/findings/projectile-feasibility.md). Real shooterless arrow/dragon, same-tick and pre-spawn evidence does not verify player-owned native dragon damage suppression or semantic head/native-damage classification. T09b supplies the next actor prerequisite; the partial merge does not unblock #9 or accept M0. |
...
| T09 — Gameplay validation | In progress (T09a complete; T09b active) | [#2 runner](https://github.com/Kav-K/OnlyDragons/issues/2), [PR #16](https://github.com/Kav-K/OnlyDragons/pull/16), [#20 protocol player](https://github.com/Kav-K/OnlyDragons/issues/20) | Runner controls passed and merged; see [positive/negative calibration](../../dev/agent-paper-tests.md#accepted-calibration-evidence). T09b requires actual protocol events, positive/negative runs, and both owned JVMs cleaned up. Human visuals/authenticated multiplayer and feature-specific scenarios remain separate gates. |
...
No milestone M0–M5 is accepted yet. The next focus is #6 combat math, final #4
item integration, #7 equipment/play after items, and T09b so T04 can resume its
player-owned experiments. A task's
...
| 2026-09-05 | T09b / [#20](https://github.com/Kav-K/OnlyDragons/issues/20), In progress | Add an optional per-run offline protocol actor only in a newly created disposable loopback profile under the user's isolated-test authorization. Preserve default/human authentication, include both JVMs in resource admission, and require client plus Paper evidence. | Local agent owns implementation; final positive/early-exit/timeout Paper evidence and integration remain pending. The owned-listener helper integrated through partial PR #22 at 586a170 is a component prerequisite, not a dependency on full T04 acceptance. T09b enables later player-owned T04 checks without accepting them or blocking independent #7 service work. |
...
Final clean runtime revision `0dd0fa72c24ee419370382bdbe81d780a4a27a94`
includes ordinary merges of main through `7c6d843`, retaining projectile and item
registrations and all four owned-resource cleanup requirements. The wrapper
`bash ./gradlew build --console=plain` and both runner builds passed on JDK
25.0.4.1: 74 production tests (20 new combat cases), zero failures/errors/skips,
and API isolation. The Python runner contract suite passed all 27 tests.

`python3 scripts/agent-tests/paper_test.py --scenario combat-accounting` returned
exit 0 for run `33d78b17c5e04d00bc7b11e3308269d6`, Paper
...
port 54977 was confirmed closed afterward. Artifact SHA256 values:

- Production: `1ca28a2c3b79115e2b60e842cb748c50acf0aacf3f92a3e98a5098da43a7756f`
- Companion: `be1870bd76a26b758e9806e9a7aa25753aeb324ce7b99334be4c545559ef0285`

The earlier clean `a4673cf` run passed 30 assertions before main integration;
the run above is final acceptance evidence. Later evidence/PR-link commits are
documentation only. Main `11b3124` was subsequently merged: its planning-only
updates were reconciled without changing production code, scenario inputs or
validation tooling, so the clean runtime evidence above remains applicable.
Lead review, final-head CI and merge remain pending.
